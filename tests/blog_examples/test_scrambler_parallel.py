...
def test_scrambler_runner():
    ...
    sources = [proj_path / "blog_examples" / "scrambler_parallel.sv"]

    runner = get_runner("verilator")
    runner.build(
        sources=sources,
        hdl_toplevel="scrambler_parallel",
        always=True,
        waves=True,
        build_args=[
            "--trace-fst",
    ...
    runner.test(
        ...
        waves=True,
        test_args=["--trace-file", "../wave/blog_examples/scrambler_parallel.fst"],
    )
...
async def test_scrambler(dut):
    ...
    dut.state_in.value = (1 << 58) - 1

    await Timer(1, "ns")

    data_out = int(dut.data_out.value)
    state_out = dut.state_out.value
